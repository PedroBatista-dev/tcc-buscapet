...
import DashboardAnimalService from '../DashboardAnimalService';
...
let dashboardAnimal: DashboardAnimalService;

describe('DashboardAnimal', () => {
  beforeEach(async () => {
    ...
    dashboardAnimal = new DashboardAnimalService(fakeAnimalsRepository);
  ...
  it('Deve ser capaz de filtrar os animais', async () => {
    const retorno = await dashboardAnimal.execute({
      user_id: user.id,
      isOng: user.isOng,
      text: 'teste',
    });

    expect(retorno).toEqual('teste - Pingo - true');
  });
});
